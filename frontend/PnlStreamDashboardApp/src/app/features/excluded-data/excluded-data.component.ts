...
@Component({
  ...
  template: `
    <div class="excluded-records-container">
      <mat-card class="data-card">
        <mat-card-header>
          <mat-card-title>Excluded Data (Invalid Records)</mat-card-title>
          <button mat-raised-button color="warn" (click)="openDownloadDialog()">
        ...
        <mat-card-content>
          <div class="table-container">
            <table mat-table [dataSource]="dataSource" class="excluded-records-table">
              <!-- SourceSystem Column -->
              ...
              <!-- ValidationReasons Column -->
              <ng-container matColumnDef="validationReasons">
                <th mat-header-cell *matHeaderCellDef>Validation Reason</th>
                <td mat-cell *matCellDef="let element">{{ element.validationReasons || 'N/A' }}</td>
              </ng-container>
          ...
          <div *ngIf="dataSource.data.length === 0" class="no-data">
            <p>No invalid records received yet</p>
          </div>
        </mat-card-content>
      </mat-card>
    </div>
...
export class ExcludedDataComponent implements OnInit, OnDestroy {
  @ViewChild(MatPaginator) paginator!: MatPaginator;

  displayedColumns: string[] = ['sourceSystem', 'portfolioNumber', 'pnlAmount', 'createdAt', 'lastUpdatedAt', 'validationReasons'];
  ...
  ngOnInit(): void {
    ...
    this.signalRService.invalidRecords$
      .pipe(takeUntil(this.destroy$))
      .subscribe((records) => {
        console.log('ExcludedDataComponent received records:', records.length);
  ...
  private loadPage(pageIndex: number, pageSize: number): void {
    ...
    this.reportService
      .getInvalidRecordsPaginated(paginationRequest)
      .pipe(takeUntil(this.destroy$))
      .subscribe(
        (response) => {
  ...
  private downloadReport(startDate: Date, endDate: Date): void {
    this.reportService.downloadInvalidReport(startDate, endDate).subscribe(
      (blob) => {
        const filename = `invalid-pnl-report-${new Date().getTime()}.csv`;
